...
export class ManualService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.client.on('message', async (topic, message) => {
      console.log(
        `Received a message on topic "${topic}": ${message.toString()}`,
      );
      try {
        if (topic.startsWith('/valve_control/log/')) {
          ...
          this.client.publish(`/valve_control/check/${data.device}`, payload, {
            qos: 1,
          });
        }
      ...
      if (
        topic.startsWith('/valve_control/start/') ||
        topic.startsWith('/valve_control/end/')
      ) {
        ...
        if (this.logSize > MAX_LOG_SIZE) {
          this.logStream.end(() => {
            ...
            const newFileName = `${parts.join(
              '.',
            )}_${new Date().toISOString()}.${ext}`;
            fs.renameSync(this.logFileName, newFileName);
            this.logStream = null;
            this.logSize = 0;
            console.log(
              `Renamed log file: ${this.logFileName} -> ${newFileName}`,
            );
          });
        }
      }
  ...
  async getAllManuals(user: { [key: string]: any }): Promise<Manual_Entity[]> {
  ...
  async showLogManual(device?: string): Promise<string> {
    ...
    const logs = device
      ? data.split('\n').filter((log) => log.includes(device))
      : data.split('\n');
    return logs.join('\n');
  }
  ...
  async update(id: number, manual: Manual_Entity): Promise<number> {
    ...
    await this.client.publish(
      `/valve_control/manual/${manual.device}`,
      Mqtt_payload,
      { qos: 1 },
    );
    return 200;
